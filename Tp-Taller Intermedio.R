'En el taller se estuvo trabajando con un archivo en formato .csv sobre algas en la región de California, 
estudiando sus características según los datos disponibles.
...
Los datos propios corresponden a la pesquería de corvina en Argentina. La corvina pertenece al grupo
variado costero. 
...
longitud (lon), latitud (lat), tf (temperatura en el fondo), sf (salinidad en el fondo), cavcorvkg 
(captura de corvina por kilo), indicejun (indice de abundancia de juveniles).'


"- - - - - - - - - - INSTALARA Y CARGAR LIBRERIAS - - - - - - - - - - - - - - -" 
...
# 2. Si no funciona, reinstalar units específicamente
install.packages("units", type = "binary")  # En Windows/Mac
...
# Cargar datos
dm=read_excel("C:/Users/AA/Desktop/adrianr/curso.hackaton/Datos juveniles corvina.xlsx")
...
tf         =as.numeric(dm$tf)
sf         =as.numeric(dm$sf)
capcorvkg  =as.numeric(dm$capcorvkg)
indicejun  =as.numeric(dm$indicejun)

dm=data.frame(anio,lon,lat,tf,sf,capcorvkg,indicejun)

'Tal como vimos en el curso, aquí tenemos para cada variable su histograma y boxplot. Primero consideramos
los datos juntando todos los años, luego haremos cada gráfico por separado por año'
...
" - - -graficamos por año el indice de abundancia  - -  "
